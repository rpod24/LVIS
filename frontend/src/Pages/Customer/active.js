...
function Active() {
  ...
  const fetchFacilitys = async (page, searchTerm) => {
    try {
      const currentPageResponse = await axios.get(`http://${BASE_URL}/manifest?p=${page}&search=${searchTerm}`);
      const nextPageResponse = await axios.get(`http://${BASE_URL}/manifest?p=${page + 1}&search=${searchTerm}`);
  ...
  const handleRowClick = (facility) => {
    navigate('/Manifest/Active/'+facility);
  };
