...
function Assembly() {
  ...
  const fetchFacilitys = async (page, searchTerm) => {
    try {
      const currentPageResponse = await axios.get(
        `http://${BASE_URL}/manifest/assembly/?p=${page}&search=${searchTerm}`,
          config
      );
      const nextPageResponse = await axios.get(
        `http://${BASE_URL}/manifest/assembly/?p=${page + 1}&search=${searchTerm}`
      );
  ...
  const handleRowClick = (facility) => {
    navigate("/Manifest/Assembly/" + facility);
  };
        ...
        <tbody>
          {facilities
            .filter((facility) => {
              ...
              return facility.status == "Assembly" ? facility : null;
            })
            .map((facility) => (
              <tr
                key={facility._id}
